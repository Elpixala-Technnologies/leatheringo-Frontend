    ...
    return (
        <section className="my-4 mx-2">
            ...
            <div className="grid gap-1 grid-cols-1 md:grid-cols-1 lg:grid-cols-1">
                {filterProductData && filterProductData?.slice(0, 4)?.map((product, index) => {
                    return (
                        <div className="cardBody flex items-center justify-around p-4 border bg-white hover:border-red-500 color-b rounded-md duration-300 transform  shadow-sm hover:-translate-y-1.5 border-t border-slate-100 hover:bg-red-10 hover:text-red-500" key={index}>
                            ...
                            <div className="productInfo p-2">
                                <p className="productName text-[0.8rem]">{product?.name.slice(0, 15) + ".."}</p>
